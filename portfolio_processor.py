...
def calculate_daily_metrics(date, open_positions, price_data, portfolio_value, total_cash_in_cumulative, total_cash_out_cumulative, i, running_peak):
    current_value = 0
    total_cost = 0
    ...
    # Store cumulative cash flows for the day
    ...
    # Calculate Cumulative Cash Flow Adjusted Return
    ...
    current_value_for_mdd = portfolio_value.loc[date, 'Current Value']
    if current_value_for_mdd > running_peak:
        running_peak = current_value_for_mdd
    
    drawdown = 0.0
    if running_peak > 0:
        drawdown = (running_peak - current_value_for_mdd) / running_peak
    portfolio_value.loc[date, 'Drawdown'] = round(drawdown, 4)
    
    return portfolio_value, running_peak
    ...
    if i > 0:
        ...
        if not daily_transactions_for_twr.empty:
            ...
            if adjusted_start_value != 0:
                portfolio_daily_return = (portfolio_value.loc[date, 'Current Value'] - adjusted_start_value) / adjusted_start_value
                portfolio_value.loc[date, 'Portfolio Daily Return'] = round(portfolio_daily_return, 4)
                portfolio_value.loc[date, 'TWR'] = round(portfolio_value.loc[prev_trading_day, 'TWR'] * (1 + portfolio_daily_return), 4)
            else:
                portfolio_value.loc[date, 'Portfolio Daily Return'] = 0.0
            ...
            if portfolio_value.loc[prev_trading_day, 'Current Value'] != 0:
                portfolio_daily_return = (portfolio_value.loc[date, 'Current Value'] / portfolio_value.loc[prev_trading_day, 'Current Value']) - 1
                portfolio_value.loc[date, 'Portfolio Daily Return'] = round(portfolio_daily_return, 4)
                portfolio_value.loc[date, 'TWR'] = round(portfolio_value.loc[prev_trading_day, 'TWR'] * (1 + portfolio_daily_return), 4)
            else:
                portfolio_value.loc[date, 'Portfolio Daily Return'] = 0.0
        ...
        if prev_trading_day in price_data[spy_ticker].index and date in price_data[spy_ticker].index:
            ...
            portfolio_value.loc[date, 'SPY Daily Return'] = round(spy_daily_return, 4)
            portfolio_value.loc[date, 'SPY_TWR'] = round(portfolio_value.loc[prev_trading_day, 'SPY_TWR'] * (1 + spy_daily_return), 4)
        else:
            portfolio_value.loc[date, 'SPY Daily Return'] = 0.0
